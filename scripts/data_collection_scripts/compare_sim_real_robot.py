...
class ExperimentalData:
    ...
    def load_from_file(cls:ExperimentalData, file_path:Path):
        ...
        return cls(measured_jp=data_dict["measured_jp"], measured_cp=data_dict["measured_cp"])
...
def plot_cartesian_errors(robot_data: ExperimentalData, sim_data:ExperimentalData):
    cartesian_err = calculate_position_error(T_RG=robot_data.measured_cp, T_RG_actual=sim_data.measured_cp)
    rot_err = calculate_orientation_error(T_RG=robot_data.measured_cp, T_RG_actual=sim_data.measured_cp)
...
def analyze_robot_error():
    ...
    real_robot_root = "./data/experiments/repeatability_experiment_rosbag01/01-11-2023-20-33-24"
    real_robot_root= Path(real_robot_root)
    sim_robot_root = "./data/experiments/simulated_robot/08-11-2023-17-17-17"
    sim_robot_root = Path(sim_robot_root)

    robot_data = ExperimentalData.load_from_file(file_path=real_robot_root / "record_001.csv")
    sim_data = ExperimentalData.load_from_file(file_path=sim_robot_root / "record_001.csv")
